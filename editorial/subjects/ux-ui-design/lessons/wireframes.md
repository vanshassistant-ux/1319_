...
A wireframe is a low-detail layout of a screen that shows structure, not style: where the content and controls go, what is most important, and how much space each part takes. Colors, fonts, and images are deliberately left out, because the point is to decide the skeleton before anyone invests in the look. Wireframes are cheap on purpose — quick to draw, easy to change, and cheap to throw away — so teams settle structure questions before any code exists.
...
- **wireframe** — A low-detail layout of a screen that shows where content and controls go, with visual style deliberately left out.
- **structure** — The skeleton of a screen: what is on it, where each element sits, and how the parts relate to one another.
- **hierarchy** — The ordering of a screen's parts by importance, so the most important content stands out first.
- **spacing** — The empty gaps between a screen's elements that shape how grouped or separated they feel.
- **placeholder** — A stand-in marking where real content will go, such as a gray box with an X for an image.
- **annotation** — A note on a wireframe that explains how a part behaves or why the design chose it.
- **mockup** — A visual representation of a screen that includes color, text, and imagery, showing how it will look.
- **prototype** — An interactive model of a product used to test how people behave with it; the wireframe's interactive sibling.
- **low-fidelity** — Rough and simplified, focused on structure rather than polish, like a quick hand sketch.
- **high-fidelity** — Detailed and close to the final design, with accurate sizes and real content in place.
...
- **Treating a wireframe as a visual design.** Colors, fonts, and images are deliberately absent from a wireframe. The Interaction Design Foundation defines wireframes as basic visual representations that outline structure and layout without focusing on design details. If a team is debating brand colors, they have moved past the wireframe stage.
- **Calling a wireframe a prototype.** A wireframe is static structure; a prototype is interactive behavior. The Interaction Design Foundation states that wireframing is distinct from prototyping because prototyping deals more with testing interactivity, and Nielsen Norman Group notes that interactive prototypes must define a response for each user action. Prototypes are their own sibling topics.
- **Skipping wireframes to save time.** Structure questions answered in code are the expensive ones. The Interaction Design Foundation notes that resolving issues at the wireframe stage avoids expensive changes later, particularly during development — and Nielsen Norman Group adds that ripping up code is very expensive while ripping up a prototype is not.
- **Polishing the first wireframe.** Messiness is a feature, not a flaw: Nielsen Norman Group says messy is completely fine and expected, especially for low-fidelity wireframes, because the point is to invite feedback while change is still free. A wireframe is a question, not a delivery.

## Compare / Contrast

- **Wireframe vs. Prototype** — A wireframe is static structure: a low-detail layout of what goes on a screen and where. A prototype is interactive behavior: a model that responds to clicks and taps so teams can test how people use it. Prototypes are sibling topics; this lesson only needs the boundary.
- **Wireframe vs. Mockup** — A wireframe shows structure — placement, hierarchy, and spacing — with no color, images, or detailed design. A mockup shows visual design — color, text, shapes, and imagery — to showcase how the screen will look. Skeleton first, then skin.
- **Low-fidelity wireframe vs. High-fidelity wireframe** — A low-fidelity wireframe is a rough, quick sketch using grayscale placeholders to focus on structure. A high-fidelity wireframe uses accurate sizes and real images and may include limited interactivity, approaching the final design.
...
- C. A clickable model of a product used to test how people interact with it.
- D. A written plan describing the steps a user takes from one screen to the next.

**Key: A.** The Interaction Design Foundation defines wireframes as basic visual representations of a user interface that outline structure and layout without focusing on design details like colors, fonts, or imagery. A styled screen is a mockup, a clickable model is a prototype, and a written step-by-step plan is a flow, not a wireframe.

**Q2.** Which of the following does a wireframe deliberately leave out?

- A. The placement of the search box and the navigation bar.
- B. The relative importance of the headline over the sidebar.
- C. Final colors, fonts, and photographic images.
- D. The spacing between the product image and its description.

**Key: C.** Wireframes deliberately omit visual style: the Interaction Design Foundation notes they do not focus on design details like colors, fonts, or imagery. Placement, hierarchy, and spacing are exactly what a wireframe shows.

**Q3.** A team shows stakeholders a gray page of boxes and lines, with a note attached saying the confirmation message will appear in the top box. A stakeholder asks, 'Where are the colors and photos?' Which response best applies this lesson's ideas?

- A. The colors and photos are missing because the design is unfinished and behind schedule.
- B. The colors and photos are deliberately absent, because the wireframe tests structure — placement, hierarchy, and spacing — before style decisions are made.
- C. The colors and photos are listed in the annotation, which records every visual detail of the screen.
- D. The team should skip wireframes and build the real screen so stakeholders can see the colors and photos.

**Key: B.** The gray boxes are the point: wireframes exist to settle structure questions first. The Interaction Design Foundation describes wireframes as basic visual representations that do not focus on colors, fonts, or imagery. Annotations explain behavior and rationale, not visual details, and skipping wireframes moves cheap questions into expensive code.

**Q4.** A product manager argues the team should skip wireframes because 'we already know what the app should look like.' Which statement best evaluates that argument using this lesson's ideas?

- A. It is correct: if the team can imagine the screen, drawing its skeleton is wasted effort.
- B. It is correct for structure, because structure is decided in code, not on paper.
- C. It is wrong, because every project must produce an annotated wireframe for every screen.
- D. It is risky: structure questions are cheapest to answer on paper, and skipping wireframes moves them into code, where changes are expensive.

**Key: D.** The economic argument is that structure decisions affect everything that follows, and resolving them at the wireframe stage avoids expensive changes later, particularly during development — as the Interaction Design Foundation states, and Nielsen Norman Group sharpens: ripping up code is very expensive. Option C overstates the rule; wireframes are a tool, not a mandatory ritual.

**Q5.** A colleague says, 'A wireframe is just a rough prototype, so once we make the links clickable, it becomes a mockup.' Which statement best analyzes that claim?

- A. It mixes the levels: wireframes, mockups, and prototypes differ by what they carry — structure, visual design, and interactivity — so adding clicks to a wireframe makes it a prototype, not a mockup.
- B. It is correct: the three terms describe the same artifact at different levels of polish.
- C. It is correct that clickable links make a mockup, because both are visual artifacts.
- D. It is wrong because a wireframe can never include any interactivity at any level of fidelity.

**Key: A.** The Interaction Design Foundation distinguishes the three: wireframes focus on layout with no color or imagery, mockups add visual elements to showcase appearance, and prototypes are experimental, interactive models for testing interaction. Interactivity is the prototype's defining feature, so clickable links move the artifact toward a prototype, not a mockup. Option D overcorrects — high-fidelity wireframes may include limited interactivity.

## Sources

- Nielsen Norman Group — Kelley Gordon, "How to Draw a Wireframe (Even if You Can't Draw)" (June 20, 2021). https://www.nngroup.com/articles/draw-wireframe-even-if-you-cant-draw/
- Interaction Design Foundation — "What is Wireframing?". https://www.interaction-design.org/literature/topics/wireframing
- Interaction Design Foundation — "What are Mockups?". https://www.interaction-design.org/literature/topics/mockup
...
- Layout (ux-ui-design:foundations:layout)

## Editorial Metadata

- Topic ID: ux-ui-design:foundations:wireframes
