...
Deductive reasoning aims at necessity: if its premises are true, a valid deductive form makes a false conclusion impossible. Validity is therefore conditional. It does not say that the premises are actually true, that the conclusion is actually true, or that the conclusion is persuasive. Consider: If a device is unplugged, it receives no power. The device is unplugged. Therefore it receives no power. The conclusion follows if the premises are true.

A counterexample tests a proposed deductive form. Find a case where all premises are true but the conclusion is false. If such a case exists, the form is invalid. For example, from “If something is a dog, it is a mammal” and “Rin is not a dog,” one cannot infer that Rin is not a mammal: Rin could be a cat. The premises would be true while the conclusion false. Formal proof systems supply more precise techniques, but the counterexample idea gives an accessible first test.
...
- **Logic:** the study of reasoning and of whether premises support a conclusion.
- **Inference:** a reasoning connection from premises to a conclusion.
...
- **Calling a valid argument true:** Validity concerns the form of a deductive inference, not whether its premises are accurate.
- **Thinking a true conclusion proves good reasoning:** Check whether the stated reasons support it.
- **Demanding certainty from induction:** Inductive evidence can justify probability without guaranteeing a result.
- **Ignoring counterexamples:** A relevant case with true premises and false conclusion defeats a claimed deductive form.

## Compare / Contrast

| A | B | Distinction |
| --- | --- | --- |
| Validity | Truth | Support-preserving deductive form versus accuracy of a statement. |
| Deduction | Induction | Necessary support if premises are true versus probabilistic support. |
...
1. **What does validity describe?** A deductive form in which true premises cannot have a false conclusion. **Answer: A.**
2. **Which question tests an inference rather than premise accuracy?** If the premises were true, would the conclusion follow? **Answer: B.**
3. **A student says, “If it rains, the field is wet. The field is wet. So it rained.” What best challenges this inference?** A sprinkler could make the field wet even if it did not rain. **Answer: C.**
4. **Which claim about induction is most accurate?** It can make a conclusion probable without making it certain. **Answer: D.**
5. **Why can a true conclusion still come from a weak argument?** The stated premises may fail to support it even if it happens to be true. **Answer: B.**

## Sources

- OpenStax, [Arguments](https://openstax.org/books/introduction-philosophy/pages/5-3-arguments) and [Types of Inferences](https://openstax.org/books/introduction-philosophy/pages/5-4-types-of-inferences).
