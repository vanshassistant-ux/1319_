...
A string method is an operation written after a string value and a dot, such as `name.lower()` or `message.strip()`. Methods can transform text, look for text, or break text into pieces. Because Python strings are immutable, a method that changes the text's content returns a result instead of editing the original value. Store or use that returned result when you need the changed text.
...
A method is an operation accessed through an object with dot notation. In `text = 'Ada'`, `text.lower()` calls the `lower` method on the string currently bound to `text`. The value before the dot is the receiver. Parentheses signal the call; some methods need no arguments, while others receive information in the parentheses. A method name is not a magic property of every Python value. It is behavior provided by the value's type, so this lesson concerns methods supplied by `str` values.
...
- **method:** An operation provided by an object's type and called with dot notation.
- **receiver:** The value before the dot in a method call, such as `text` in `text.lower()`.
- **immutable:** Unable to have its existing contents changed in place.
- **normalize:** To make values follow a chosen consistent form, such as removing edge whitespace and using one case.
- **substring:** A consecutive sequence of characters that occurs within a larger string.
- **delimiter:** A character or string used to mark the boundary between text pieces.
- **sentinel value:** A special returned value that signals a condition; `find` uses `-1` for an absent substring.
...
String methods are card tools that produce a fresh card or report something about the card. Python does not make physical cards, and some methods return lists or numbers rather than strings. The picture is meant to show that transformations produce a result without altering the original string.
...
- **Mistake:** Calling `text.strip()` and expecting `text` itself to lose whitespace. **Correction:** Keep the returned string, for example `text = text.strip()` or `cleaned = text.strip()`.
- **Mistake:** Treating `-1` from `find` as a valid found position. **Correction:** Read `-1` as not found; use `in` when the program only needs a membership test.
- **Mistake:** Using `join` as though it were a list method. **Correction:** Call it on the separator string: `', '.join(items)`.
- **Mistake:** Assuming `split(',')` understands every comma-containing format. **Correction:** It separates literal commas; use a format-aware tool when quoted or escaped fields matter.

## Compare / Contrast

- `text.strip()` computes a trimmed result, while `text = text.strip()` makes the name refer to that result.
- `text.find(part)` reports a position or `-1`, while `part in text` directly answers whether the substring occurs.
- `text.split(',')` turns one delimited string into a list; `','.join(parts)` combines iterable string elements with a delimiter.
...
1. In `label.lower()`, `label` is the receiver of the method call.
2. `'  Ada  '.strip()` evaluates to `'Ada'`.
3. `'notebook'.find('book')` evaluates to `4`.
4. `'/'.join(parts)` combines strings in `parts` with slashes.
5. `raw.strip()` leaves `raw` unchanged when the returned new string is not saved.

## Sources

- Python Software Foundation, [Built-in Types — Text Sequence Type: str and String Methods](https://docs.python.org/3/library/stdtypes.html#string-methods)
...
- `python-programming:foundations:strings`
- `python-programming:foundations:type-conversion`
- `python-programming:foundations:functions`
- `python-programming:foundations:files`

## Editorial Metadata

Original EliExplains synthesis. Research status: source-verified. Rights status: reference-only official documentation; no source prose adapted. Examples verified with `python3` on 2026-08-19.
