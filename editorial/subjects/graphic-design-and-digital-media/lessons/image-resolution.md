...
A raster image is a grid of pixels. Its pixel dimensions are how many pixels wide and tall it is; its resolution is how densely those pixels are packed when it is shown or printed, measured in pixels per inch (ppi). Print size equals pixel dimension divided by ppi, so the same file prints larger but coarser at a lower ppi. Print aims for about 300 ppi; the old 72 ppi screen figure is a historical leftover. Enlarging a raster invents pixels and loses quality.
...
- **Treating pixel dimensions and resolution (ppi) as the same thing.** Pixel dimensions are the fixed count of pixels (e.g. 3000 x 2400). Resolution is how densely those pixels are placed when output. The same dimensions can print at any size depending on the ppi you choose.
- **Using "ppi" and "dpi" as if they always mean the same thing.** PPI is the pixel density of an image or screen; DPI is the density of a printer's ink dots. They are linked through screening but are different quantities. Casual usage blurs them, so recognize a "300 dpi image" as meaning 300 ppi.
- **Saving web images at 72 ppi because "screens are 72 ppi."** 72 ppi is a historical figure from the original Macintosh. Modern screens are far denser (HiDPI/Retina). For on-screen use, what matters is the pixel dimensions, not a stored ppi tag.
- **Enlarging a small image and expecting it to look sharp.** Upscaling makes the software invent pixels by interpolation, which softens and blurs rather than adding real detail. Start with enough pixels for the largest output size instead.
- **Assuming a low ppi means the file is broken.** A low ppi just means the pixels are spread over a large area. The same file at a smaller size, or a higher ppi, can be perfectly sharp; print size = pixel dimension / ppi tells you what size it supports.

## Compare / Contrast

- **Pixel dimensions vs. Resolution (ppi):** Pixel dimensions are the fixed number of pixels in the file; resolution is how densely those pixels are laid down when displayed or printed. Dimensions are set; ppi is chosen at output.
- **PPI vs. DPI:** PPI counts image or display pixels per inch; DPI counts a printer's ink dots per inch. A printer converts pixels into dot patterns, so the two are related through screening but are not the same measurement.
- **300 ppi (print) vs. 72 ppi (legacy screen):** About 300 ppi is a live convention grounded in halftone screening and eye acuity; 72 ppi is a historical leftover from the first Macintosh and 72 points per inch, no longer descriptive of modern high-density screens.
- **Lowering ppi to enlarge vs. Resampling up to enlarge:** Lowering ppi spreads the existing pixels over more area (bigger but coarser, no new data invented); resampling up manufactures new pixels by interpolation, which blurs and cannot recover real detail.
...
1. **A raster image's pixel dimensions are listed as 3000 x 2400. What does that number describe?**
   - How many ink dots per inch the printer will use
   - *The count of pixels across the image's width and height* ✓
   - The physical size of the print in inches
   - The image's density in pixels per inch

   Pixel dimensions are simply the count of pixels across (width) and down (height), a fixed property of the file. Printer ink density is dpi, physical print size is measured in inches and depends on the chosen ppi, and pixel density in ppi is the separate quantity called resolution.

2. **What is the precise distinction between PPI and DPI?**
   - PPI is used only for printers and DPI only for screens
   - They are identical terms and always fully interchangeable
   - *PPI is the pixel density of an image or display, while DPI is the density of the ink dots a printer lays down* ✓
   - PPI measures a file's physical size and DPI measures its storage size

   PPI (pixels per inch) is the density of image or display pixels; DPI (dots per inch) is properly the density of a printer's ink or toner dots, which the printer produces by converting pixels into dot patterns via screening. Option A reverses the terms, B ignores a real distinction (they are only loosely used as synonyms), and D misstates both.

3. **A 3000 x 2400 px photo prints at 10 x 8 inches at 300 ppi. If you print the same file at 150 ppi instead, what happens?**
   - *It prints larger, 20 x 16 inches, but looks coarser* ✓
   - It prints at the same size but with sharper detail
   - It gains extra pixels and looks crisper
   - It cannot be printed at all below 300 ppi

   Print size = pixel dimension / ppi, so at 150 ppi the image is 3000/150 by 2400/150 = 20 by 16 inches. The same pixels now cover four times the area, so it prints larger but coarser. Lowering ppi never adds pixels or detail, and there is no hard rule forbidding printing below 300 ppi.

4. **A print shop needs a 300 ppi image for an 8 x 10 inch print. What pixel dimensions must the file have?**
   - 300 x 300 px
   - 8 x 10 px
   - 800 x 1000 px
   - *2400 x 3000 px* ✓

   Pixels needed = inches x ppi, so 8 x 300 = 2400 and 10 x 300 = 3000, giving 2400 x 3000 px (7.2 megapixels). The other options are far too small: 800 x 1000 px would only yield 100 ppi at that size, and 300 x 300 or 8 x 10 px are nowhere near enough.

5. **Why does enlarging (upscaling) a small raster image reduce its quality?**
   - The file format silently changes when the image is enlarged
   - The monitor lowers its own pixel density to compensate
   - Printers automatically reject any file under 300 ppi
   - *The software must invent new pixels by interpolation, estimating them from neighbors rather than recovering real detail, which softens edges and adds artifacts* ✓

   Upscaling requires more pixels than were captured, so the software interpolates new ones from surrounding pixels (nearest-neighbor, bilinear, bicubic). Those are approximations, not recovered detail, so the result blurs or shows artifacts. The format does not change on its own, the monitor's density is unrelated, and printers do not universally reject low-ppi files.

## Sources

- Graphic Design and Print Production Fundamentals, Ch. 5.2 Raster Image Processing — BCcampus / Graphic Communications Open Textbook Collective (CC BY 4.0). https://opentextbc.ca/graphicdesign/
- Pixel density — Wikipedia (CC BY-SA 4.0). https://en.wikipedia.org/wiki/Pixel_density
- About Points and Pixels as Units — Henri Sivonen (reference). https://hsivonen.fi/units/
- Image scaling — Wikipedia (CC BY-SA 4.0). https://en.wikipedia.org/wiki/Image_scaling
...
- **Rights:** One CC BY open textbook (attributed, no wording reproduced) plus three reference-only sources (two Wikipedia CC BY-SA articles, one technical article); all prose original.
- **Transformation:** Pixels, pixel dimensions versus resolution, the ppi/dpi distinction, the 300 ppi and 72 ppi conventions, and resampling synthesized from the sources; worked example computed and verified in Bash.
