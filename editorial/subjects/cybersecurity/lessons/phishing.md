...
Phishing is a social attack: fraudulent messages that pose as a trusted person or organization to trick you into revealing information or clicking harmful links. It leans on fake urgency, impersonation, and fake login pages. The common forms are email phishing, smishing (text messages), vishing (phone calls), and spear phishing (targeted at specific people). If a message pressures you to act fast, don't click, don't reply — report it and verify through a channel you know is real.
...
- **phishing** — A form of social engineering in which fraudulent messages pose as a trusted person or organization to trick recipients into revealing sensitive information or clicking harmful links.
- **smishing** — Phishing delivered by text message: SMS messages disguised as trustworthy communications, such as a delivery notice or bank alert.
- **vishing** — Phishing conducted over voice calls, in which callers impersonate a trusted organization to extract sensitive information.
- **spear phishing** — A targeted phishing attempt that uses research about a specific person or organization to make a customized, believable message.
- **credential phishing** — A phishing attempt aimed at stealing usernames and passwords, usually by sending the victim to a fake login page that mimics a real one.
- **spoofed link** — A link whose visible text or sender appears legitimate but whose real destination is an attacker-controlled site, often a look-alike login page.
- **multi-factor authentication (MFA)** — A login process that requires more than one kind of evidence — such as a password plus a one-time code — so that a phished password alone is not enough to enter an account.
...
- **Judging a message only by its grammar.** CISA warns that AI tools now let some phishing messages arrive with perfect grammar and spelling. Check the sender address, the real destination of links, and the pressure to act instead.
- **Clicking the link "just to see if it's real."** Clicking is what the phisher wants — the link may open a fake login page or harmful content. Inspect the link's destination without opening it, or verify through a channel you know is real.
- **Replying to a suspicious message to ask if it is legitimate.** A reply confirms your address is active and invites more attempts. Report the message and contact the real organization through contact information you already have.
- **Believing phishing only happens to careless people.** Phishing exploits trust and emotion, and attackers push for quick decisions; even careful people can be deceived. The defense is the habit of slowing down, not a personal immunity.
- **Using the contact details inside the suspicious message to verify it.** CISA and the FTC advise contacting the company through a number or website you already know — the phone number in a suspicious message may belong to the attacker.

## Compare / Contrast

- **Phishing vs. malware** — Phishing is deception aimed at a person's decision; malware is software that harms a device. A phishing message can be the delivery method for malware, but the two are different kinds of threat — and malware has its own lesson.
- **Smishing vs. vishing** — Both are phishing through a phone, but smishing arrives as a text message and vishing as a voice call; smishing usually asks you to click, vishing usually asks you to talk.
- **Broad phishing vs. spear phishing** — Broad phishing is a mass message that works when enough recipients bite; spear phishing is researched and customized for one specific person or role, which makes it harder to recognize.
...
- C. A security rule that blocks unexpected connections to a network.
- D. An attack that tries many password guesses until one works.

**Key: A.** NIST's glossary and CISA define phishing as fraudulent solicitation that masquerades as a legitimate business or trusted person to collect sensitive information, often through harmful links. Option B describes self-spreading software, C describes a firewall rule, and D describes brute-force guessing — none involve the deceptive message at the heart of phishing.

**Q2.** Ravi gets a text that looks like a delivery notice from a courier he has used, warning that a package will be returned within hours unless he clicks a link to reschedule. The link leads to an unfamiliar site. Which type of phishing is this, and which two tactics does it combine?

- A. Vishing; it combines a voice call with a fake payment page.
- B. Spear phishing; it combines deep research on Ravi with a personalized offer.
- C. Smishing; it combines impersonation of a trusted business with fake urgency.
- D. Email phishing; it combines a generic greeting with a shortened link.

**Key: C.** A text-message phishing attempt is smishing — SMS plus phishing — and the courier disguise plus the tight deadline are exactly the impersonation and urgency that CISA and Microsoft describe. Vishing is voice, spear phishing is researched and targeted, and this message arrives as a text, not email.

**Q3.** A coworker says a suspicious email passed their "grammar check," so it must be legitimate. Based on current CISA guidance, why is grammar alone a weak test?

- A. Because grammar has never been considered a sign of phishing.
- B. Because legitimate companies routinely send messages with errors.
- C. Because the message came from an address the coworker recognized.
- D. Because AI tools now let phishers produce messages with perfect grammar, so the other signs — urgency, mismatched links, requests for information — matter more.

**Key: D.** CISA notes that poor grammar used to be a common phishing sign but that, in the era of AI, some messages now have perfect grammar and spelling, and it advises looking at the other indicators. A and B contradict the sources, and C changes the subject — a recognized-looking address can still be imitated.

**Q4.** Priya receives an email claiming to be from her bank, asking her to verify her password through a link. What is the best general response?

- A. Click the link to check whether the page is real.
- B. Do not click or reply; report the message, and contact the bank through a phone number or website she knows is genuine.
- C. Reply to the sender asking them to prove they work for the bank.
- D. Forward the email to friends so they can watch for the same scam.

**Key: B.** FTC and CISA guidance is consistent: don't click links or attachments, don't reply, report the message, and verify through an independent channel — never the contact details inside the message. Clicking is exactly what the phisher wants, replying confirms a live address, and forwarding spreads the threat.

**Q5.** Which statement best explains why phishing succeeds, even with careful people?

- A. Phishing only works on people who lack technical knowledge.
- B. Phishing succeeds mainly when security software is out of date.
- C. Phishing exploits trust and emotion — urgency, impersonation, and fear — so the deception can work on anyone who is rushed or fooled.
- D. Phishing succeeds only because targets ignore official warnings.

**Key: C.** Microsoft's security guidance is explicit that attackers create a false sense of trust and exploit emotions like fear, and that even the most perceptive people can be deceived. The other options blame ignorance, software, or warnings alone, which misses the point that phishing targets a human decision.
...
- U.S. Federal Trade Commission (FTC) — "How to Recognize and Avoid Phishing Scams". https://consumer.ftc.gov/articles/how-recognize-and-avoid-phishing-scams
- Microsoft Security — "What is phishing?". https://www.microsoft.com/en-us/security/business/security-101/what-is-phishing
- National Institute of Standards and Technology (NIST) — "phishing — Glossary | CSRC". https://csrc.nist.gov/glossary/term/phishing
- National Institute of Standards and Technology (NIST) — "NIST Special Publication 800-63B: Authentication and Authenticator Management". https://pages.nist.gov/800-63-4/sp800-63b.html

## Related Topics

- Social Engineering (cybersecurity:foundations:social-engineering)
- Security Awareness (cybersecurity:foundations:security-awareness)
- Threats (cybersecurity:foundations:threats)
- Password Security (cybersecurity:foundations:password-security)
- Multi-Factor Authentication (cybersecurity:foundations:mfa)

## Editorial Metadata

- Topic ID: cybersecurity:foundations:phishing
